...
Each story has a checkbox. The agent marks a story `[x]` after it is implemented and tests pass, then moves on to the next unchecked story.  `Note:` if you run this and want to run again you will have to remove the 'x' from each user story.
...
Open a Terminal session in the project directory and run:
...
1. **Clone the repo and remove the generated code:**
   ...
   rm -rf api/ ui/
   ```
  The `api` and `ui` folders will be generated by the Ralph Loop 
...
3. **Run Ralph Loop** in the project directory and kick off the loop:
