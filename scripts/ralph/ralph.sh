...
# Always operate from the repo root so the agent can read the Markdown instructions
cd "$PROJECT_ROOT"
...
if ! grep -q '\- \[ \]' "$USER_STORIES_FILE"; then
  cat <<'MSG'
No unchecked stories left in USER_STORIES.md.
Reset the checkboxes (change '- [x]' back to '- [ ]') if you want Ralph to rebuild the project.
MSG
  exit 0
fi
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  if [[ "$TOOL" == "amp" ]]; then
    ...
    # Codex: run non-interactively with sandbox + approval overrides
